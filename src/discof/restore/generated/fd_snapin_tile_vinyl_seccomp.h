...
#ifndef HEADER_fd_src_discof_restore_generated_fd_snapin_tile_vinyl_seccomp_h
#define HEADER_fd_src_discof_restore_generated_fd_snapin_tile_vinyl_seccomp_h
...
#endif
static const unsigned int sock_filter_policy_fd_snapin_tile_vinyl_instr_cnt = 23;

static void populate_sock_filter_policy_fd_snapin_tile_vinyl( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd ) {
  FD_TEST( out_cnt >= 23 );
  struct sock_filter filter[23] = {
    ...
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_madvise, /* check_madvise */ 9, 0 ),
    /* allow msync based on expression */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_msync, /* check_msync */ 10, 0 ),
    /* allow exit based on expression */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_exit, /* check_exit */ 11, 0 ),
...
//  check_madvise:
    ...
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, MADV_SEQUENTIAL, /* RET_ALLOW */ 5, /* RET_KILL_PROCESS */ 4 ),
//  check_msync:
    /* load syscall argument 2 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[2])),
