...
send_shred( fd_shred_ctx_t                 * ctx,
  ...
  fd_snp_meta_t meta = fd_snp_meta_from_parts( dest->is_secure ? FD_SNP_META_PROTO_V1 : FD_SNP_META_PROTO_UDP, /* app_id */ 0, dest->ip4, dest->port );
  if( reuse_prev_pkt ) {
...
#define NONNULL( x ) (__extension__({                                        \
  ...
  for( ulong i=0UL; i<ctx->adtl_dests_retransmit_cnt; i++) {
    ...
    ctx->adtl_dests_retransmit[ i ].is_secure = 1U; /* TODO */
  }
  ...
  for( ulong i=0UL; i<ctx->adtl_dests_leader_cnt; i++) {
    ...
    ctx->adtl_dests_leader[i].is_secure = 1U; /* TODO */
  }
