...
#endif
static const unsigned int sock_filter_policy_archiver_writer_instr_cnt = 16;

static void populate_sock_filter_policy_archiver_writer( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd, unsigned int archive_fd ) {
  FD_TEST( out_cnt >= 16 );
...
//  lbl_1:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, archive_fd, /* RET_ALLOW */ 5, /* lbl_2 */ 0 ),
//  lbl_2:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
