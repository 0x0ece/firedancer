...
#ifndef HEADER_fd_src_disco_archiver_generated_archiver_playback_seccomp_h
#define HEADER_fd_src_disco_archiver_generated_archiver_playback_seccomp_h
...
#endif
static const unsigned int sock_filter_policy_archiver_playback_instr_cnt = 17;

static void populate_sock_filter_policy_archiver_playback( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd, unsigned int archive_fd ) {
  FD_TEST( out_cnt >= 17 );
...
//  check_read:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, archive_fd, /* RET_ALLOW */ 3, /* RET_KILL_PROCESS */ 2 ),
//  check_fsync:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
